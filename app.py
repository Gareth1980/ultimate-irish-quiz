...
def edit_question(question_and_answer_id):
    the_question =  mongo.db.question_and_answer.find_one({"_id": ObjectId(question_and_answer_id)})
...
def delete_question(question_and_answer_id):
    mongo.db.question_and_answer.remove({'_id': ObjectId(question_and_answer_id)})
    return redirect(url_for('get_questions'))
